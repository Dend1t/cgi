...
def fire(event):
    global x1, y1, x2, y2, score

    center_x = (x1 + x2) / 2
    center_y = (y1 + y2) / 2
    if m.sqrt((event.x-center_x)**2 + (event.y-center_y)**2) < 250:
...
for i in range(8):
    canvas.create_oval(0+30*i, 110+30*i, 500-30*i, 610-30*i, outline="blue", width=10, tags='target',fill='lightgray')

canvas.create_line(0, 350, 500, 350, tags='target', width=2)
